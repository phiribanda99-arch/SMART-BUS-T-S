        ...
        if (!$scheduleId || $seatNumber === '' || !in_array($paymentMethod, $allowedMethods, true)) {
            ...
            try {
                ...
                $scheduleStmt = $pdo->prepare(
                    'SELECT s.*, r.route_code, r.origin, r.origin_district, r.destination, r.destination_district, b.seating_capacity
                ...
                if (!ctype_digit((string) $seatNumber) || (int) $seatNumber < 1 || (int) $seatNumber > (int) $schedule['seating_capacity']) {
                    throw new RuntimeException('Seat number must be a valid seat within the bus capacity.');
                }
